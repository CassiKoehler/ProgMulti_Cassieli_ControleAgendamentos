...
@section('content')
    <div class="p-6 text-gray-900 dark:text-gray-100 flex flex-col items-center justify-center">
        ...
        <form action="{{ route('agendamentos.store') }}" method="POST"
            class="w-full max-w-2xl bg-gray-800 p-6 rounded shadow space-y-4">
            @csrf
                    ...
                    <tbody class="text-white">
                        @forelse($agendamentos as $agendamento)
                            <tr class="border-t border-gray-700">
                                ...
                                <td class="py-2 px-4">{{ \Carbon\Carbon::parse($agendamento->data_hora)->format('d/m/Y H:i') }}
                                </td>
                                ...
                                <td class="py-2 px-4">
                                    ...
                                    <form action="{{ route('agendamentos.destroy', $agendamento->id) }}" method="POST"
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" onclick="return confirm('Deseja excluir este agendamento?')"
